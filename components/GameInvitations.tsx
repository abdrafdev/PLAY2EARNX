...
const GameInvitations: React.FC<ComponentProps> = ({ invitations, game, label }) => {
  ...
  const handleResponse = async (accept: boolean, invitation: InvitationStruct, index: number) => {
    ...
    if (invitation.responded) {
      toast.warning('You have already responded to this invitation')
      return
    }
        ...
        new Promise<void>((resolve, reject) => {
          respondToInvite(accept, invitation, index)
            ...
            .catch((error: any) => {
              ...
              reject(error.message || 'Failed to respond to invitation')
            })
        }),
        {
          pending: 'Approve transaction...',
          success: accept ? 'Invitation accepted successfully 👌' : 'Invitation rejected successfully',
          error: {
            render({ data }) {
              // Display the actual error message from blockchain service
              return typeof data === 'string' ? data : 'Failed to respond to invitation'
            },
          },
        }
      )
